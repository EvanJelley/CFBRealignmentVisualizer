...
useEffect(() => {
    ...
    setChartData({
      labels: conferenceList.map((conference) => conference.conference === selectedConference ? conference.year : null),
      datasets: [
        {
          label: 'Average Distance Between Schools',
          data: conferenceList.map((conference) => conference.conference === selectedConference ? conference.avgDistanceBetweenSchools : null),
        },
        {
          label: 'Average Distance from Center',
          data: conferenceList.map((conference) => conference.conference === selectedConference ? conference.avgDistanceFromCenter : null),
        },
      ],
      options: {
        scales: {
          xAxes: [{
            type: 'time', // Use 'time' scale if your data is time-based
            time: {
              unit: 'year', // Adjust according to your data (e.g., 'month', 'year')
              tooltipFormat: 'YYYY', // Adjust the tooltip format
              displayFormats: {
                quarter: 'YYYY' // Adjust display formats according to your needs
              }
            },
            ticks: {
              autoSkip: true,
              maxTicksLimit: 20 // Adjust this to control the maximum number of ticks displayed
            }
          }],
          yAxes: [{
            ticks: {
              beginAtZero: true // Adjust Y-axis to begin at zero
            }
          }]
        }
      }
